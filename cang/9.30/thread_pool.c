...
void *routine(void *arg)//任务执行函数
{
	...
	while(1)
	{
		...
		*/
		...
		if(pool->waiting_tasks == 0 && pool->shutdown == true) //假如没有任务与关闭线程的话 就解锁退出线程
		{
...
bool init_pool(thread_pool *pool, unsigned int threads_number)//初始化线程池链表，并一个创建线程来运行线程池管理函数
{
	...
	pool->tids = malloc(sizeof(pthread_t) * MAX_ACTIVE_THREADS);  //线程池最大线程数 20  
    ...
    //复制最大线程数
	pool->max_waiting_tasks = MAX_WAITING_TASKS;  //最多可以添加的任务为 1000 
	pool->waiting_tasks = 0; //任务为0
	...
	for(i=0; i < pool->active_threads; i++)   //创建active_threads个线程来运行任务执行函数
	{
		if(pthread_create(&((pool->tids)[i]), NULL,
					routine, (void *)pool) != 0)  //把整个线程池传递给 routine
		{
...
bool add_task(thread_pool *pool,void *(*do_task)(void *arg), void *arg)//添加任务
{
	...
	new_task->do_task = do_task;  //给新的任务结点赋值void *  func(void *arg)
	new_task->arg = arg;  //给任务的参数
	new_task->next = NULL;
...
{ 
	if(removing_threads == 0)  //removing_threads 需要删除的数量
		return pool->active_threads;

	int remaining_threads = pool->active_threads - removing_threads;

	remaining_threads = remaining_threads > 0 ? remaining_threads : 1;//为什么要留1，why?   （留下一个线程去处理任务）
	//remaining_threads 删除后剩下的线程
	int i;
	...
	{
		pool->active_threads = i+1;  //把剩下的线程数量赋值给  活跃的   
		return i+1;
	}
}
